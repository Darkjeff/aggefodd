...
if (GETPOST('action', 'none')) {
	...
	if ($action == 'get_infos_from_training') {
		$fk_training= GETPOST('fk_training', 'none');
		if (!empty($fk_training)) {
			$sql="SELECT cat.duree,cat.fk_product, cat.fk_nature_action_code, p.ref FROM ".MAIN_DB_PREFIX."agefodd_formation_catalogue cat";
			...
			if (!empty($resql)) {
				...
				$data['fk_nature_action_code']=$obj->fk_nature_action_code;
				$data['ref_product']=$obj->ref;
				$data['result'] = 1;
			}
		}
	} elseif ($action == 'get_nb_place') {
		/*
			...
			if (!empty($resql)) {
				...
				if ($fk_place>0) {
					...
					if (!empty($resql_place)) {
						...
						elseif (!empty($place->nb_place)) $data['nb_place'] =  $place->nb_place;
					}
				}
