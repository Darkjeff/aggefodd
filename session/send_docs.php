...
$hookmanager->initHooks(array('agefodd_send_docs', 'formmail'));
	...
	if ($action == 'send' && empty($addfile) && empty($removedfile) && empty($cancel)) {
		...
		if (! empty($send_to)) {
			...
			foreach ( $receiver as $id_receiver ) {
				...
				} elseif (preg_match('/^(\d+)_user$/', $id_receiver, $m)) {
					$id_receiver = intval($m[1]);
					$receiverUser = new User($db);
					if ($receiverUser->fetch($id_receiver) > 0) {
						$sendto[$id_receiver . '_user'] = trim($receiverUser->firstname . ' ' . $receiverUser->lastname) . ' <' .$receiverUser->email . '>';
					} else {
						setEventMessage($langs->trans('UserNotFound', $id_receiver), array(), 'errors');
					}
				} else {
					$contactstatic = new Contact($db);
					$contactstatic->fetch($id_receiver);
		...
		if (is_array($sendto) && count($sendto) > 0) {
			...
			foreach ( $sendto as $send_contact_id => $send_email ) {
				...
				if ($models == 'certificate_completion') {
					if (empty($subject))
						$langs->transnoentities('AgfCourrierCompletionCertificate') . ' ' . $object->ref;
					...
					$actionmsg2 = $langs->trans('ActionCERTIFICAT_COMPLETION_SENTBYMAIL');
				}
				...
				if ($mailfile->error) {
					...
					if ($result) {
						...
						} elseif ($models == 'certificate_completion') {
							$result = $interface->run_triggers('CERTIFICATE_COMPLETION_SENTBYMAIL', $object, $user, $langs, $conf);
						}
						if ($result < 0) {
							$error ++;
						...
						if ($mailfile->error) {
                            ...
                            if (!empty($conf->global->MAIN_DISABLE_ALL_MAILS)) setEventMessage('No mail sent. Feature is disabled by option MAIN_DISABLE_ALL_MAILS', 'errors');
                            if (!empty($mailfile->errors)) setEventMessages('Mail not sent', $mailfile->errors, 'errors');
						}
					}
				}
			}
...
if (! empty($id)) {
	...
	if ($result>0) {
		...
		if (
			...
			|| $action == 'presend_pedago_recueil'
			|| $action == 'presend_presence'
			|| $action == 'presend_presence_direct'
			|| $action == 'presend_presence_empty'
			|| $action == 'presend_presence_landscape_bymonth'
			|| $action == 'presend_convention'
			|| $action == 'presend_attestation'
			|| $action == 'presend_cloture'
			|| $action == 'presend_convocation'
			|| $action == 'presend_conseils'
			|| $action == 'presend_accueil'
			|| $action == 'presend_mission_trainer'
			|| $action == 'presend_trainer_doc'
			|| $action == 'presend_attestationendtraining'
			|| $action == 'presend_attestationpresencetraining'
			|| $action == 'presend_presence_landscape_empty'
			|| $action == 'presend_reg_int'
			|| $action == 'presend_linked_certificate_completion_trainee'
		) {
			...
			if ($action == 'presend_presence') {
                ...
                $filename = 'linked_certificate_completion_trainee_'.$agf->id.'_'.$socid.'.pdf';
			}
			...
			if ($mode == 'init') {
				...
				if ($action == 'presend_convention') {
					...
					if (! empty($conf->global->AGF_ADD_PROGRAM_TO_CONVMAIL)) {
						...
						//  on test si une copie de formation_catalogue (session_catalogue :  fiche péda recueil modifiée)  existe pour cette session
						$agfSessionCat = new SessionCatalogue($db);
						//On teste l'existence d'un clone pour renvoyer la bonne fiche pédago
						$iscloned = $agfSessionCat->isCloned('', '', '', '', '', array(' AND sc.fk_session = '.$agf->id));
						if($iscloned){
							$res= $agfSessionCat->fetchSessionCatalogue($agf->id);
							if ($res){
								$agfSessionCat->generatePDAByLink();
							}
							$filename = 'fiche_pedago_recueil_' . $agf->id. '.pdf';
						}else{
							$agfTraining = new Formation($db);
							$res = $agfTraining->fetch($agf->fk_formation_catalogue);
							if ($res){
								$agfTraining->generatePDAByLink();
							}
							$filename = 'fiche_pedago_' . $agf->fk_formation_catalogue . '.pdf';
					...
					if (empty($conf->global->AGF_MERGE_ADVISE_AND_CONVOC)) {
						...
						// add Rules of procedure to convention mail
						$dirRegInt = $conf->agefodd->multidir_output[$conf->entity].'/place/'.$agf->fk_session_place;
						$listFiles = dol_dir_list($dirRegInt, 'files', 0, '^reglement_(.*)\.pdf$');
						if (is_file($dirRegInt. '/' . $listFiles[0]['name'])) {
							$file = $dirRegInt. '/' . $listFiles[0]['name'];
					...
					// add Rules of procedure to mail
					$dirRegInt = $conf->agefodd->multidir_output[$conf->entity].'/place/'.$agf->fk_session_place;
					$listFiles = dol_dir_list($dirRegInt, 'files', 0, '^reglement_(.*)\.pdf$');
					if (is_file($dirRegInt. '/' . $listFiles[0]['name'])) {
						$file = $dirRegInt. '/' . $listFiles[0]['name'];
				...
				} elseif ($action == 'presend_conseils')
				{
					...
					// add Rules of procedure to advises mail
					$dirRegInt = $conf->agefodd->multidir_output[$conf->entity].'/place/'.$agf->fk_session_place;
					$listFiles = dol_dir_list($dirRegInt, 'files', 0, '^reglement_(.*)\.pdf$');
					if (is_file($dirRegInt. '/' . $listFiles[0]['name'])) {
						$file = $dirRegInt. '/' . $listFiles[0]['name'];
				...
				}elseif ($action == 'presend_reg_int')
				{
					...
					// add advises to Rules of procedure mail
					$filename = 'conseils_' . $agf->id . '.pdf';
					$file = $conf->agefodd->dir_output . '/' . $filename;
					if (file_exists($file)) {
						$formmail->add_attached_files($file, basename($file), dol_mimetype($file));
				...
				elseif ($action == 'presend_attestationendtraining') {
                    ...
                    $formmail->param['models'] = 'certificate_completion';
                    $formmail->param['pre_action'] = 'presend_linked_certificate_completion_trainee';
                    $recipient = GETPOST('socid', 'int');

                    $socmail = new Societe($db);
                    $resSocmail = $socmail->fetch($recipient);
                    if ($resSocmail < 0) {
                        dol_print_error($db);
                        exit;
                    } elseif ($resSocmail == 0) {
                        setEventMessage('NotFound');
                    }

                    //Récupérer l'email du destinataire
                    if(!empty($socmail->email)) {
                        $formmail->withtofree = $socmail->email;
                    }

                    //Récupérer l'objet du mail
                    $formmail->withtopic = $agf->formintitule;

                    //Définir le contenu du mail
                    $formmail->withbody = $langs->trans('AgfSendLinkedCertificateCompletionBodyTitle', '__FORMINTITULE__');
                    $formmail->withbody .= '</br></br>';
                    $formmail->withbody .= $langs->trans('AgfSendLinkedCertificateCompletionBodyContent', '__FORMINTITULE__');
                    $formmail->withbody .= '</br></br>';
                    $formmail->withbody .= $langs->trans('AgfSendLinkedCertificateCompletionBodySignature', '__FORMINTITULE__');
					$file_array[]=$file;
...
//			$formmail->withcancel = 1;
            ...
            if ($action == 'presend_presence') {
                ...
                if ($num > 0) {
                    foreach ( $agftrainersess->lines as $formateur ) {
                        if ($formateur->email != '') {
                            $withto[$formateur->socpeopleid] = $formateur->lastname . ' ' . $formateur->firstname . ' (' . $langs->trans('AgfFormateur') . ')' . ' <' . $formateur->email . '>';
                        }
                ...
                if ($agf->type_session && $socid) {
                    ...
                    if (! $result_opca) {
                        ...
                        if (! empty($contactstatic->email)) {
                            $withto[$agf_opca->fk_socpeople_OPCA] = $contactstatic->lastname . ' ' . $contactstatic->firstname . ' (' . $langs->trans('AgfMailTypeContactOPCA') . ')' . ' <' . $contactstatic->email . '>';
                    ...
                    if (! empty($contactstatic->email)) {
                        $withto[$agf->fk_socpeople_OPCA] = $contactstatic->lastname . ' ' . $contactstatic->firstname . ' (' . $langs->trans('AgfMailTypeContactOPCA') . ')' . ' <' . $contactstatic->email . '>';
                ...
                if ($agf->sourcecontactid > 0) {
                    ...
                    $withto[$agf->sourcecontactid] = $contactstatic->lastname . ' ' . $contactstatic->firstname . ' (' . $langs->trans('AgfSessionContact') . ')' . ' <' . $contactstatic->email . '>';
                ...
                if (! empty($agf->fk_soc)) {
                    ...
                    foreach ( $soc_contact as $id => $mail ) {
                        ...
                        if (! empty($contactstatic->email)) {
                            $withto[$id] = $contactstatic->lastname . ' ' . $contactstatic->firstname . ' (' . $langs->trans('AgfSessionContact') . ')' . ' <' . $contactstatic->email . '>';
			...
			} elseif ($action == 'presend_pedago' || $action == 'presend_pedago_recueil') {
				...
				if (empty($sessiontrainerid)) {
					...
					if ($result < 0) {
						...
						foreach ( $agf_trainer_session->lines as $linetrainer ) {
							...
							$multiselectKey = $linetrainer->socpeopleid;
							if (!$linetrainer->socpeopleid && $linetrainer->userid) {
								$multiselectKey = $linetrainer->userid . '_user';
							}
							$withto[$multiselectKey] = $linetrainer->lastname . ' ' . $linetrainer->name . ' - ' . $linetrainer->email;
							$withtoname[$multiselectKey] = $linetrainer->lastname . ' ' . $linetrainer->name;
						}
					}
				} else {
					$agf_trainer_session = new Agefodd_session_formateur($db);
				...
				if (! empty($agf->sourcecontactid)) {
					$contactstatic = new Contact($db);
					$contactstatic->fetch($agf->sourcecontactid);
                    $withto[$agf->sourcecontactid] = $contactstatic->lastname . ' ' . $contactstatic->firstname . ' (' . $langs->trans('AgfMailTypeContactCommanditaire') . ')' . ' <' . $contactstatic->email . '>';
					$withtoname[$agf->sourcecontactid] = $contactstatic->getFullName($langs);
					if (! empty($contactstatic->socname)) {
						$withtocompanyname[$contactstatic->socid] = $contactstatic->socname;
					}
				...
				if ($socid > 0) {
					...
					foreach ( $soc_contact as $id => $mail ) {
						$contactstatic = new Contact($db);
						$contactstatic->fetch($id);
                        $withto[$id] = $contactstatic->lastname . ' ' . $contactstatic->firstname . ' (' . $langs->trans('AgfMailTypeContactTrainee') . ')' . ' <' . $contactstatic->email . '>';
						$withtoname[$id] = $contactstatic->getFullName($langs);
						if (! empty($contactstatic->socname)) {
							$withtocompanyname[$contactstatic->socid] = $contactstatic->socname;
						}
				...
				foreach ( $agf_trainnee->lines as $line ) {
					if (! empty($line->email) && (! empty($line->fk_socpeople))) {
						if (! array_key_exists($line->fk_socpeople, $withto)) {
							$withto[$line->fk_socpeople] = $line->nom . ' ' . $line->prenom . ' (' . $langs->trans('AgfMailTypeContactCommanditaire') . ')' . ' <' . $line->email . '>';
				...
				foreach ( $agf_trainnee->lines as $line ) {
					if (! empty($line->email) && (! empty($line->fk_socpeople))) {
						if (! array_key_exists($line->fk_socpeople, $withto)) {
							$withto[$line->fk_socpeople] = $line->nom . ' ' . $line->prenom . ' (' . $langs->trans('AgfMailTypeContactTrainee') . ')' . ' <' . $line->email . '>';
							$withtoname[$line->fk_socpeople] = $line->nom . ' ' . $line->prenom;
						}
				...
				$formmail->withtofree = 1;
			} elseif ($action == "presend_conseils" || $action == "presend_reg_int") {
			...
			if ($action == 'presend_pedago' || $action == 'presend_pedago_recueil') {
