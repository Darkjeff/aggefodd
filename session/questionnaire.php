...
if (!empty($massaction) && $massaction == 'send' && !empty($arrayofselected)) {
	...
	if ($objQuestionnaire->fetch($idQuestionnaire) > 0) {
		foreach ($arrayofselected as $inv_selected) {
			...
			$content = prepareMailContent($invuser, $idQuestionnaire, 'HTML');
...
if ($action == 'linkquestionnaire') {
...
} elseif ($action == "addInvitationsSta" && !empty($idQuestionnaire)) {
	if (!empty($user->rights->agefodd->questionnaire->send)) {
		...
		if ($objQuestionnaire->fetch($idQuestionnaire) > 0) {
			...
			if (!empty($toselect)) {
				$addRes = addInvitationsTrainnee($objQuestionnaire, $user, $toselect, $date_limite_reponse, $addLog);
	...
	$action = 'prepareAddInvitationsSta';
} elseif ($action == 'addInvitationsTrainer' && !empty($idQuestionnaire)) {
	if (!empty($user->rights->agefodd->questionnaire->send)) {
		$objQuestionnaire = new Questionnaire($db);
		if ($objQuestionnaire->fetch($idQuestionnaire) > 0) {
			$toselect = GETPOST('toselect', 'array');
			...
			$date_limite_reponse = strtotime($date_limite_year . '-' . $date_limite_month . '-' . $date_limite_day);
			if (!empty($toselect)) {
				$addRes = addInvitationsTrainner($objQuestionnaire, $user, $toselect, $date_limite_reponse, $addLog);
	...
	$action = 'prepareAddInvitationsTrainer';
}
	...
	<div class="agf_row">
		<div class="agf_col-2 agf_left_nav">
			...
			<?php if (!empty($user->rights->agefodd->questionnaire->link)) { ?>
				<form name="answerQuestionnaire" method="POST"
					  action="<?php print $_SERVER['PHP_SELF'] . '?id=' . $id; ?>">
					<?php
					print getQuestionnaireSessionListForm($agf->id, 'notlinked', 'questionnaire');
					?>
					<button class="button" type="submit" name="action"
							value="linkquestionnaire"><?php print $langs->trans('Add') ?></button>
				</form>
			<?php } ?>
		...
		<div class="agf_col-10">
			<?php if (empty($objQuestionnaire)) {
				...
				<div class="agf_row agf_title_head">
					...
					<h3 class="pull-left"><small><?php print $objQuestionnaire->ref; ?></small>
						- <?php print $objQuestionnaire->title; ?></h3>

					<?php if (!empty($user->rights->questionnaire->read)) { ?>
						<a class="agf_btn agf_btn-app pull-right classfortooltip"
						   href="<?php print dol_buildpath('/questionnaire/card.php', 1) . '?id=' . $linkedQuestionnnaire->id; ?>"
						   title="<?php print $langs->trans('agfQuestCardShortHelp'); ?>">
					...
					<?php if (!empty($user->rights->agefodd->questionnaire->send)) { ?>
						<?php if ($action == 'prepareAddInvitationsSta') { ?>
							<a class="agf_btn agf_btn-app pull-right classfortooltip"
							   href="<?php print dol_buildpath('/agefodd/session/questionnaire.php', 1) . '?id=' . $agf->id . '&amp;idQuestionnaire=' . $linkedQuestionnnaire->id; ?>"
							   title="<?php print $langs->trans('agfQuestAnswerShortHelp'); ?>">
								<i class="fa fa-reply-all"></i>
								<?php print $langs->trans('agfQuestAnswerShort'); ?>
							</a>
						<?php } else { ?>
							<a class="agf_btn agf_btn-app pull-right classfortooltip"
							   href="<?php print dol_buildpath('/agefodd/session/questionnaire.php', 1) . '?id=' . $agf->id . '&amp;idQuestionnaire=' . $linkedQuestionnnaire->id . '&amp;action=prepareAddInvitationsSta'; ?>"
							   title="<?php print $langs->trans('agfQuestSendShortStaHelp'); ?>">
								<i class="fa fa-plus-circle"></i>
								<?php print $langs->trans('agfQuestSendShortSta'); ?>
							</a>
							<a class="agf_btn agf_btn-app pull-right classfortooltip"
							   href="<?php print dol_buildpath('/agefodd/session/questionnaire.php', 1) . '?id=' . $agf->id . '&amp;idQuestionnaire=' . $linkedQuestionnnaire->id . '&amp;action=prepareAddInvitationsTrainer'; ?>"
							   title="<?php print $langs->trans('agfQuestSendShortTrainerHelp'); ?>">
								<i class="fa fa-plus-circle"></i>
								<?php print $langs->trans('agfQuestSendShortTrainer'); ?>
							</a>
						<?php } ?>
					<?php } ?>
					...
					<?php if (!empty($user->rights->agefodd->questionnaire->link)) { ?>
						<a class="agf_btn agf_btn-app pull-right classfortooltip"
						   href="<?php print dol_buildpath('/agefodd/session/questionnaire.php', 1) . '?id=' . $agf->id . '&amp;questionnaire=' . $linkedQuestionnnaire->id . '&amp;action=unlink'; ?>"
						   title="<?php print $langs->trans('agfQuestUnlinkShortHelp'); ?>">
				...
				<div class="agf_list_wrap">
					...
					if (($massaction == "addInvitationsSta" || $massaction == "addInvitationsTrainer") && !empty($confirmmassaction) && !empty($idQuestionnaire)) {
						if (!empty($user->rights->agefodd->questionnaire->send)) {
							$form = new Form($db);
					...
					if ($action == 'prepareAddInvitationsSta') {
						_printRenderQuestionnaireParticipantsList($objQuestionnaire, $agf);
					} elseif ($action == 'prepareAddInvitationsTrainer') {
						_printRenderQuestionnaireTrainersList($objQuestionnaire, $agf);
					} else {
...
/**
 * @param Questionnaire $object Questionnaire
 * @param Agsession $session Session
 * @return void
 */
function _printRenderQuestionnaireParticipantsList(Questionnaire $object, Agsession $session)
{
	...
	$url = $_SERVER['PHP_SELF'] . '?id=' . $session->id . '&idQuestionnaire=' . $object->id . '&amp;action=prepareAddInvitationsSta';
	...
	$param = array(
		...
		, 'list'      => array(
			...
			'massactions' => array(
				'addInvitationsSta' => $langs->trans('AgfQuestionnaireInvite')
			)
		)
		, 'hide'      => array('rowid')
		, 'title'     => array(
 ...
 * @return void
 */
function _printRenderQuestionnaireTrainersList(Questionnaire $object, Agsession $session)
{
	global $db, $langs, $hookmanager;

	$url = $_SERVER['PHP_SELF'] . '?id=' . $session->id . '&idQuestionnaire=' . $object->id . '&amp;action=prepareAddInvitationsTrainer';

	$formcore = new TFormCore($url, 'form_list_questionnaire_trainer', 'POST');

	$r = new Listview($db, 'questionnaire-session_trainer-list');


	$sql = "SELECT";
	$sql .= " s.rowid, s.fk_agefodd_formateur,  s.trainer_status, iu.rowid invitation_id, ";
	$sql .= " IFNULL(utrainer.lastname,strainer.lastname) as nom,";
	$sql .= " IFNULL(utrainer.firstname,strainer.firstname) as prenom,";
	$sql .= " IFNULL(utrainer.civilite,strainer.civility) as civilite,";
	$sql .= " strainer.fk_soc,";
	$sql .= " IFNULL(utrainer.email,strainer.email) as mail";
	$sql .= " FROM " . MAIN_DB_PREFIX . "agefodd_session_formateur s";
	$sql .= " JOIN " . MAIN_DB_PREFIX . "agefodd_formateur trainer ON (trainer.rowid = s.fk_agefodd_formateur ) ";
	$sql .= " LEFT JOIN " . MAIN_DB_PREFIX . "user utrainer ON (utrainer.rowid = trainer.fk_user AND trainer.type_trainer='user') ";
	$sql .= " LEFT JOIN " . MAIN_DB_PREFIX . "socpeople strainer ON (strainer.rowid = trainer.fk_socpeople AND trainer.type_trainer='socpeople' ) ";
	$sql .= " LEFT JOIN " . MAIN_DB_PREFIX . "quest_invitation_user iu ON (iu.fk_element = s.fk_agefodd_formateur AND iu.type_element = 'agefodd_formateur' ) ";
	$sql .= " WHERE s.fk_session = " . $session->id . ' AND iu.rowid IS NULL ';

	$trainnerCardUrl = dol_buildpath('agefodd/trainer/card.php', 1) . '?id=@fk_agefodd_formateur@';
	$trainnerCardLink = '<a href="' . $trainnerCardUrl . '" >@val@</a>';
	...
	$param = array(
		...
		, 'subQuery'  => array()
		, 'link'      => array(
			'rowid'    => $trainnerCardLink
			, 'prenom' => $trainnerCardLink
			, 'nom'    => $trainnerCardLink
			, 'mail'   => $trainnerCardLink
		)
		, 'type'      => array()
		, 'search'    => array()
		, 'translate' => array()
		, 'list'      => array(
			...
			'massactions' => array(
				'addInvitationsTrainer' => $langs->trans('AgfQuestionnaireInvite')
			)
		)
		, 'hide'      => array('rowid')
		, 'title'     => array(
...
/**
 * @param Questionnaire $object Questionnaire
 * @param Agsession $session Agsession
 * @return void
 */
function _printRenderQuestionnaireGuestsList(Questionnaire $object, Agsession $session)
{
	...
	$listViewConfig = array(
		...
		, 'search'    => array(
			...
			, 'sent'              => array('search_type' => InvitationUser::$TSentStatus, 'to_translate' => true)
			// select html, la clé = le status de l'objet, 'to_translate' à true si nécessaire
			, 'email'             => array('search_type' => true, 'table' => array('iu', 'iu'),
										   'field'       => array('email'))
