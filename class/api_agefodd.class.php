...
class Agefodd extends DolibarrApi
{
	...
	function _generateNewConv($sessid, $socid)
	{
		...
		else $this->convention->art1 .= $langs->transnoentitiesnoconv('AgfConvArt1_9') . ' ' . dol_print_date($this->session->dated) . ' ' . $langs->transnoentitiesnoconv('AgfConvArt1_10') . ' ' . dol_print_date($this->session->datef);
	...
	function documentsDownload($filename, $entity = 0)
	{
		...
		if (empty($entity))
			$entity=$conf->entity;
		elseif (!isset($conf->agefodd->multidir_output[$entity])) throw new RestException(500, "the entity provided has no folder defined");
